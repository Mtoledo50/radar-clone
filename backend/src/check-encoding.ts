...
async function main() {
  ...
  // 🟡 BLOCO 2: Gerar IDs únicos baseados no tempo (evita qualquer conflito de duplicidade)
  const timestamp = Date.now();
  const contatoIdUnico = `55119${timestamp}`;
  const documentoUnico = `123456789${timestamp.toString().slice(-4)}`;

  console.log(`📱 Criando contato teste: ${contatoIdUnico}`);
  console.log(`📄 Documento único: ${documentoUnico}`);

  // 🟢 BLOCO 3: Cria um NOVO contato (sem upsert, para evitar conflito de chave única)
