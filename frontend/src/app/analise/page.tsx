...
export default function AnalisePage() {
  ...
  // --------------------------------------------------------------------------
  // 🔧 FIX: função buscarPendentes COMPLETA (com finally e chave de fechamento)
  // --------------------------------------------------------------------------
  const buscarPendentes = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.status === 'ok') setPendentes(data.data);
        else setPendentes([]);
      } else {
        setPendentes([]); // endpoint inativo, segue sem dados
      }
    } catch {
      setPendentes([]); // erro de rede, segue sem dados
    } finally {
      setLoading(false); // 🔧 ADICIONADO
    }
  }; // 🔧 ADICIONADO — fechamento da função

  // --------------------------------------------------------------------------
  // Classificar conversa selecionada
  // --------------------------------------------------------------------------
  const handleClassificar = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3001/analise/classificar/${selecionada.id}`,
        {
          method: 'POST',
  ...
  if (loading)
    return (
      <div className="p-8 text-center text-gray-500">
        Carregando conversas pendentes...
      </div>
    );
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">
        📊 Análise e Classificação de Conversas
      </h1>
          ...
          <div className="max-h-[600px] overflow-y-auto">
            {pendentes.length === 0 ? (
              <p className="p-6 text-center text-gray-500">
                Nenhuma conversa pendente! Tudo em dia. 🎉
              </p>
            ) : (
        ...
        <div className="lg:col-span-2 bg-white rounded-lg shadow p-6">
          {selecionada ? (
                ...
                <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                  <p className="text-sm text-gray-500 mb-1">
                    ...
                    <span className="font-semibold text-gray-900">
                      {selecionada.contato.contatoId}
                    </span>
                  </p>
                  <p className="text-sm text-gray-500 mb-1">
                    Assunto:{' '}
                    <span className="font-semibold text-gray-900">
                      {selecionada.contato.ultimoAssunto || 'Não informado'}
                    </span>
                  </p>
                  <p className="text-sm text-gray-500 mb-1">
                    Data:{' '}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  >
                    ...
                    <option value="pedido_documento">
                      Pedido de documento
                    </option>
                    <option value="abertura_empresa">
                      Abertura de empresa
                    </option>
                    <option value="fechamento_mensal">
                      Fechamento mensal
                    </option>
                    <option value="problema_tecnico">Problema técnico</option>
                    <option value="so_informacao">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        complexidade: parseInt(e.target.value),
                      })
                    }
                    className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        tempoGastoMin: parseInt(e.target.value),
                      })
                    }
                    className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                  />
            ...
            <div className="h-full flex flex-col items-center justify-center text-gray-400 min-h-[400px]">
              ...
              <p className="text-lg">
                Selecione uma conversa à esquerda para classificar.
              </p>
            </div>
          )}
        </div>
