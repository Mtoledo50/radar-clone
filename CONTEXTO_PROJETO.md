...
IMEDIATO: Sprint D3 = Ranking de Níveis (gamificação Bronze→Diamante +
pódio da rede, derivado do Score C4 — ADR-058).
...
IMEDIATO: validar C1 → abrir C2 (catálogo de serviços extras com preço médio).

Fase B (Pessoas): B1 ✅ • B2 ✅ • B3 ✅ • B4 ✅ • B5 ✅ — FASE B COMPLETA.

Sprint B5 HOMOLOGADA: benchmark de cargos por setor (domínio puro ADR-051,
maiores restos, gaps VACANCY/OK/OVER, página dedicada + menu).
IMEDIATO: Fase C (Mercado) — C1 benchmark de softwares • C2 serviços extras
c/ preço médio • C3 indicadores c/ fórmula • C4 score 0–100.
Recomendado começar pela C1 (reaproveita Company.softwareStack da "Minha Empresa").

Sprint B4 HOMOLOGADA: entrevista de desligamento c/ motor rules-v1 (ADR-050) +
sub-aba Análises IA + FIX B4.1 (rota GET /turnover/resignations).
IMEDIATO: Sprint B5 = Benchmark de Cargos por Setor (domínio puro + gaps ao vivo).

Sprint B3 HOMOLOGADA: KPIs de turnover (novatos/críticos/tenure) com
flag `isCritical` em Employee + cópia histórica em Resignation (ADR-049).
KPIs da aba Rescisões vêm de endpoints reais; checkbox "Era crítico?"
no modal de rescisão.
IMEDIATO: Sprint B4 = Entrevista de Desligamento com IA (causa-raiz +
...
IMEDIATO: Sprint B2 = Distribuição por setor validada (atual × recomendado).
DEPOIS: B3→B5, Fases C→E, FD-5→FD-9.
