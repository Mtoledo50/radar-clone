...
Added
- Domínio puro CNAB 240/400 em `backend/src/billing/domain/` (parsers de retorno,
  builder de remessa, validators) — 7 testes unitários verdes.
- Schema Prisma: 4 tabelas (`cnab_arquivos`, `cnab_movimentos`, `cobranca_regras`,
  `cobranca_eventos`) + 5 enums; `CobrancaEvento.clientId` opcional.
- `BillingModule` NestJS c/ 17 endpoints: CRUD BillingInstruction (compatibilidade),
  upload de retorno CNAB (multipart), processamento de movimentos (baixa), CRUD de
  regras, execução da régua, aprovação humana (aprovar/rejeitar/enviar), históricos.
- Frontend: página "Cobrança & CNAB" c/ 4 abas (Cobranças, Remessas, Retornos,
  Régua) em `/dashboard/funcionario-digital/cobranca`.
Fixed
- Type-check do projeto inteiro (seeds + tests): arrays heterogêneos no seed-plans,
  upsert→findFirst+create/update no seed-accounts, e2e bootstrap sem supertest/types.
- CORS do backend: portas dev 3003/3005 liberadas.
Decisions
ADR-084 Domínio puro CNAB isolado (sem dependências de banco/HTTP); aprovação
humana obrigatória em toda ação automática de cobrança.
ADR-085 Arquitetura híbrida FD-5: BillingInstruction como fonte de verdade
(compatibilidade); CnabArquivo/CnabMovimento como histórico CNAB;
CobrancaRegra/CobrancaEvento como régua c/ workflow humano.
Provas
- `npm run test -- --testPathPattern=cnab240`: 1 suite, 7 passed.
- `npx tsc --noEmit`: 0 erros.
- E2E visual (porta 3003): remessa gerada/baixada, regra ativa, evento aprovado e enviado.
...
Ordem dos edits: 1) tsc --noEmit zero erros → 2) remover flag → 3) rebuild Docker
verde.
Provas
- `npx tsc --noEmit` retorna "Found 0 errors".
- `next build` mostra "Finished TypeScript in 8.5s" (sem fail).
- Todos os 5 containers do compose permanecem Up após o rebuild.


[Sprint 32 — Produção local: Radar + Site Conta Certa] 2026-08-26 — ✅ HOMOLOGADO
Added
...
ADR-081 (env de build > .env.local) • ADR-082 (scroll suave nativo em vez de hash do router).
